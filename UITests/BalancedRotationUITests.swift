...
final class BalancedRotationUITests: OpenLiftUITestCase {
    func testBalancedButtonBlocksDraftAndPersistsCold() throws {
        ...
        let apply = app.buttons["cycle.applyBalancedRotation"]
        ...
        apply.tap()
        let success = app.descendants(matching: .any).matching(identifier: "cycle.balancedSuccess").firstMatch
        ...
        receipt.name = "Balanced rotation succeeded"; receipt.lifetime = .keepAlways; add(receipt)
        app.terminate()
        app.launch()
        app.tabBars.buttons["Cycle"].tap()
        XCTAssertTrue(app.staticTexts["Clustered Hypertrophy v7"].firstMatch.waitForExistence(timeout: 5))
        XCTAssertFalse(apply.exists)
        let persisted = app.descendants(matching: .any).matching(identifier: "cycle.balancedSuccess").firstMatch
        ...
        XCTAssertTrue(nextThird.label.contains("Stair Calves"))
    }
}
