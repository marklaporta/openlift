...
final class MovementHistoryUITests: OpenLiftUITestCase {
    func testMovementDefaultSearchProgressionAndWorkoutToggle() throws {
        ...
        let total = app.staticTexts["history.allSets.summary"]
        scrollToElement(total, in: app)
        XCTAssertTrue(total.isHittable)
        XCTAssertTrue(total.label.contains("4 sets"))
        ...
        scrollToElement(total, in: app)
        XCTAssertTrue(total.isHittable, "All setups retains the total overlay")
