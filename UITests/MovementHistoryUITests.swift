...
final class MovementHistoryUITests: OpenLiftUITestCase {
    func testMovementDefaultSearchProgressionAndWorkoutToggle() throws {
        ...
        XCTAssertTrue(app.staticTexts["4 workouts · 13 sets"].exists)
        ...
        let average = app.staticTexts["history.allSets.summary"]
        scrollToElement(average, in: app)
        XCTAssertTrue(average.isHittable)
        XCTAssertTrue(average.label.contains("4 sets"))
        XCTAssertTrue(average.label.contains("All sets avg: 104.7"))
        XCTAssertTrue(app.staticTexts["All sets avg"].exists)
        let totalChart = XCTAttachment(screenshot: app.screenshot()); totalChart.name = "All sets overlay"; totalChart.lifetime = .keepAlways; add(totalChart)
        ...
        scrollToElement(average, in: app)
        XCTAssertTrue(average.isHittable, "All setups retains the average overlay")
        let totalSetups = XCTAttachment(screenshot: app.screenshot()); totalSetups.name = "All sets overlay across setups"; totalSetups.lifetime = .keepAlways; add(totalSetups)
