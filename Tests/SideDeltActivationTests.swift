...
final class SideDeltActivationTests: XCTestCase {
    ...
    private func fixture() throws -> Fixture {
        ...
        let revision = try BootstrapDataService.prepareSeatedShrugClusterRevision(modelContext: context, backupConfirmed: true)
    ...
    func testFreshSnapshotContainsLatestPreRevisionStateAndRepeatDoesNotCreateAnotherBackup() throws {
        ...
        let stale = backups.appendingPathComponent("before-third-side-delt-older.sqlite")
        ...
        let applied = try BootstrapDataService.applySideDeltRevisionWithFreshBackup(modelContext: f.context, backupDirectory: backups)
        ...
        XCTAssertFalse(try saved.fetch(FetchDescriptor<TrainingPreference>()).contains { $0.key == BootstrapDataService.sideDeltRevisionMarker })
        let repeated = try BootstrapDataService.applySideDeltRevisionWithFreshBackup(modelContext: f.context, backupDirectory: backups,
    ...
    func testSnapshotFailureAndInvalidSnapshotLeaveProgramUntouched() throws {
        ...
        XCTAssertFalse(try f.context.fetch(FetchDescriptor<TrainingPreference>()).contains { $0.key == BootstrapDataService.sideDeltRevisionMarker })
    ...
    @MainActor
    func testAdaptiveDraftIsRetainedAndRefusedBeforeBackup() throws {
        let f = try fixture()
        defer { try? FileManager.default.removeItem(at: f.root) }
        let draft = AdaptiveWorkoutSession(generatedPlanId: UUID())
        f.context.insert(draft)
        let row = AdaptiveSetEntry(adaptiveSessionId: draft.id, occurrenceId: UUID(),
            exerciseId: f.exerciseID, setIndex: 1, weight: 30, reps: 12, isLocked: true)
        f.context.insert(row)
        try f.context.save()
        XCTAssertThrowsError(try BootstrapDataService.applySideDeltRevisionWithFreshBackup(modelContext: f.context,
            snapshot: { _, _ in XCTFail("Adaptive draft must not reach backup") }))
        XCTAssertEqual(f.cycle.templateId, f.templateID)
        XCTAssertEqual(try f.context.fetch(FetchDescriptor<AdaptiveWorkoutSession>()).first?.id, draft.id)
        XCTAssertEqual(try f.context.fetch(FetchDescriptor<AdaptiveSetEntry>()).first?.id, row.id)
...
final class SideDeltOrderActivationTests: XCTestCase {
    private struct Fixture {
        let root: URL
        let container: ModelContainer
        let context: ModelContext
    ...
    private func fixture() throws -> Fixture {
        ...
        let revision = try BootstrapDataService.prepareSideDeltClusterRevision(modelContext: context, backupConfirmed: true)
        let cycle = try XCTUnwrap(try context.fetch(FetchDescriptor<ActiveCycleInstance>()).first)
        let exercise = try XCTUnwrap(try context.fetch(FetchDescriptor<Exercise>()).first)
        return Fixture(root: root, container: container, context: context, cycle: cycle,
            templateID: revision.templateId, exerciseID: exercise.id)
    ...
    func testFreshSnapshotContainsLatestPreRevisionStateAndRepeatDoesNotCreateAnotherBackup() throws {
        ...
        let backups = f.root.appendingPathComponent("revision-backups")
        try FileManager.default.createDirectory(at: backups, withIntermediateDirectories: true)
        let stale = backups.appendingPathComponent("before-side-delt-order-older.sqlite")
        try StoreBackupService.snapshot(storeAt: f.root.appendingPathComponent("default.store"), into: stale)
        let staleBytes = try Data(contentsOf: stale)
        let latestID = try addHistory(f, reps: 14)
        let applied = try BootstrapDataService.applySideDeltOrderWithFreshBackup(modelContext: f.context, backupDirectory: backups)
        XCTAssertTrue(applied.revision.didApply)
        let backup = try XCTUnwrap(applied.backupURL)
        XCTAssertNotEqual(backup, stale)
        XCTAssertTrue(StoreBackupService.isValidSnapshot(at: backup))
        ...
        XCTAssertEqual(try saved.fetch(FetchDescriptor<SetEntry>()).map(\.reps).sorted(), [10, 14])
        XCTAssertEqual(try saved.fetch(FetchDescriptor<ActiveCycleInstance>()).first?.templateId, f.templateID)
        XCTAssertFalse(try saved.fetch(FetchDescriptor<TrainingPreference>()).contains { $0.key == BootstrapDataService.sideDeltOrderMarker })
        let repeated = try BootstrapDataService.applySideDeltOrderWithFreshBackup(modelContext: f.context, backupDirectory: backups,
            snapshot: { _, _ in XCTFail("Already-applied revisions must not create backup churn") })
        XCTAssertFalse(repeated.revision.didApply)
        XCTAssertNil(repeated.backupURL)
        XCTAssertEqual(try FileManager.default.contentsOfDirectory(at: backups, includingPropertiesForKeys: nil).count, 2)
    ...
    func testSnapshotFailureAndInvalidSnapshotLeaveProgramUntouched() throws {
        ...
        let backups = f.root.appendingPathComponent("revision-backups")
        enum Injected: Error { case snapshotFailure }
        XCTAssertThrowsError(try BootstrapDataService.applySideDeltOrderWithFreshBackup(modelContext: f.context,
            backupDirectory: backups, snapshot: { _, _ in throw Injected.snapshotFailure }))
        XCTAssertThrowsError(try BootstrapDataService.applySideDeltOrderWithFreshBackup(modelContext: f.context,
            backupDirectory: backups, snapshot: { _, target in try Data("not SQLite".utf8).write(to: target) }))
        XCTAssertEqual(f.cycle.templateId, f.templateID)
        XCTAssertEqual(try f.context.fetch(FetchDescriptor<ClusterRotationState>()).count, 12)
        XCTAssertEqual(try f.context.fetch(FetchDescriptor<Session>()).count, 1)
        XCTAssertEqual(try f.context.fetch(FetchDescriptor<SetEntry>()).first?.reps, 12)
        XCTAssertFalse(try f.context.fetch(FetchDescriptor<TrainingPreference>()).contains { $0.key == BootstrapDataService.sideDeltOrderMarker })
        XCTAssertTrue(try FileManager.default.contentsOfDirectory(at: backups, includingPropertiesForKeys: nil).isEmpty)
    }
