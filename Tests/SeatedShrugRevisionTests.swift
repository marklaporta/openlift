...
final class SeatedShrugRevisionTests: XCTestCase {
    ...
    func testCatalogLabelCorrectionKeepsExistingV3IdentityDraftAndFrozenHistory() throws {
        let f = try fixture()
        let result = try BootstrapDataService.prepareSeatedShrugClusterRevision(modelContext: f.context, backupConfirmed: true)
        let template = try XCTUnwrap(try f.context.fetch(FetchDescriptor<CycleTemplate>()).first { $0.id == result.templateId })
        let shrug = try XCTUnwrap(f.exercises.first { $0.name == Program.shrugExerciseName })
        ...
        let beforePointers = try f.context.fetch(FetchDescriptor<ClusterRotationState>()).map {
            "\($0.id)|\($0.programVersionID)|\($0.positionIndex)|\($0.updatedAt)|\(String(describing: $0.lastCompletedOccurrenceID))"
        }.sorted()
        let count = f.exercises.count
        ...
        XCTAssertEqual(try f.context.fetch(FetchDescriptor<ClusterRotationState>()).map {
            "\($0.id)|\($0.programVersionID)|\($0.positionIndex)|\($0.updatedAt)|\(String(describing: $0.lastCompletedOccurrenceID))"
        }.sorted(), beforePointers)
        XCTAssertEqual(f.cycle.templateId, template.id)
