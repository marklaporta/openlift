    ...
    private static func reconcileWorkoutExportsInCleanContext(
        ...
        let revisionExports = exports.filter {
            $0.fixed_cycle?.program_identifier == FixedCycleClusterProgramService.programIdentifier
                && [2, 3, 4, 5, 6, 7].contains($0.fixed_cycle?.program_version ?? 0)
        ...
        if revisionExports.contains(where: { $0.fixed_cycle?.program_version == 6 || $0.fixed_cycle?.program_version == 7 }) {
        ...
        // the newest revision. Older exports must not downgrade the live cycle.
        let revisionGroups = Dictionary(grouping: revisionExports) { $0.fixed_cycle!.template_id }
            .values.sorted { ($0.map(\.date).max() ?? "") < ($1.map(\.date).max() ?? "") }
        for templateExports in revisionGroups {
            guard let newestRevision = templateExports.max(by: { $0.date < $1.date }),
                  let metadata = newestRevision.fixed_cycle,
                  let exportedTemplateID = UUID(uuidString: metadata.template_id),
                  let version = metadata.program_version,
                  templateExports.allSatisfy({ $0.fixed_cycle?.program_version == version }) else {
            ...
            if let existing = availableTemplates.first(where: { $0.id == exportedTemplateID }) {
                ...
                if version == 7 {
                    ...
                    revised = try FixedCycleClusterProgramService.makeTemplate(exercises: catalog, revised: true, shrugs: version >= 3, thirdSideDelt: version >= 4, inclineFirst: version == 5, chestBackRecovery: version == 6)
                }
                ...
                for export in templateExports.sorted(by: { $0.date < $1.date }) {
                    for occurrence in export.fixed_cycle?.cluster_occurrences ?? [] where occurrence.program_version_id == FixedCycleClusterProgramService.versionID(for: revised) {
            ...
            if availableTemplates.contains(where: {
                $0.id == destination.templateId && FixedCycleClusterProgramService.isProgramTemplate($0)
                    && FixedCycleClusterProgramService.versionNumber(for: $0) <= version
            }) {
                ...
                if canHydrateClusterState,
                   ...
                   export.session_id == latestClusterPreferenceExports["\(FixedCycleClusterProgramService.programIdentifier).v\(metadata.program_version ?? 0)"]?.sessionID {
                    for payload in metadata.cluster_exercise_preferences ?? [] where
                        ...
                        if payload.program_version_id == FixedCycleClusterProgramService.balancedVersionID,
                           ...
                           templatesByID[recoveredTemplateID(metadata, fallback: cycle.templateId)]?.days
                            ...
                            .first(where: { $0.position == payload.slot_position })?.exerciseId == exercise.id { continue }
                          ...
                          let occurrence = try? ClusterOccurrenceRecord(
                              ...
                              templateDayPosition: occurrencePayload.template_day_position
                                  ?? FixedCycleClusterProgramService.Cluster(
                                      ...
                                      FixedCycleClusterProgramService.templatePosition($0, step: occurrencePayload.position_index, version: occurrencePayload.program_version_id)
...
enum FixedCycleClusterProgramService {
    ...
    static let sideDeltOrderVersionID = "\(programIdentifier).v5"
    static let sideDeltOrderTemplateName = "Clustered Hypertrophy v5"
    static let sideDeltOrderIdentityKey = "openlift_clustered_hypertrophy_v5"

    static func isReservedTemplateName(_ name: String) -> Bool {
        [templateName, revisionTemplateName, shrugTemplateName, sideDeltTemplateName, sideDeltOrderTemplateName, chestBackTemplateName, balancedTemplateName].contains { name.caseInsensitiveCompare($0) == .orderedSame }
    }
    static func supports(version: Int?) -> Bool { [1, 2, 3, 4, 5, 6, 7].contains(version ?? 0) }
    static func supports(programVersionID: String) -> Bool {
        [self.programVersionID, revisionVersionID, shrugVersionID, sideDeltVersionID, sideDeltOrderVersionID, chestBackVersionID, balancedVersionID].contains(programVersionID)
    }
    ...
    static func versionNumber(for template: CycleTemplate) -> Int {
        ...
        if template.rotationPools.contains(where: { $0.key == sideDeltOrderIdentityKey }) { return 5 }
    ...
    static func isProgramTemplate(_ template: CycleTemplate?) -> Bool {
        ...
        guard let template,
              template.rotationPools.contains(where: {
                  [templateIdentityKey, revisionIdentityKey, shrugIdentityKey, sideDeltIdentityKey, sideDeltOrderIdentityKey, chestBackIdentityKey].contains($0.key) && $0.entries.isEmpty
              }) else { return false }
        ...
        if versionNumber(for: template) >= 2 {
            for position in 3...8 {
        ...
        if versionNumber(for: template) == 6 {
            expectedDays.append((15, "Cluster 1 · D", [(0, .chest), (1, .back)]))
        }
            ...
            return day.position == expected.0
                ...
                && zip(slots, expected.2).allSatisfy { slot, role in
                    slot.position == role.0
                        && slot.muscle == role.1
                        && (versionNumber(for: template) >= 2 ? slot.defaultSetCount > 0 : slot.defaultSetCount == 3)
                }
        }
    }

    static func makeTemplate(exercises: [Exercise], revised: Bool = false, shrugs: Bool = false, thirdSideDelt: Bool = false, inclineFirst: Bool = false, chestBackRecovery: Bool = false) throws -> CycleTemplate {
        ...
        if shrugs {
            for day in cluster3 where [9, 11, 13].contains(day.position) {
                day.slots.append(CycleSlot(position: 2, muscle: .traps,
                    exerciseId: try required([shrugExerciseName]).id, defaultSetCount: 2))
            }
        }
        if thirdSideDelt {
            let shoulders = inclineFirst
                ? [sideDeltExerciseName, "Super ROM Dumbbell Lateral Raise", "Cable Lateral Raise"]
                : ["Super ROM Dumbbell Lateral Raise", sideDeltExerciseName, "Cable Lateral Raise"]
        ...
        let template = CycleTemplate(
            name: inclineFirst ? sideDeltOrderTemplateName : (thirdSideDelt ? sideDeltTemplateName : (shrugs ? shrugTemplateName : (revised ? revisionTemplateName : templateName))),
            days: cluster1 + cluster2 + cluster3,
            rotationPools: [RotationPool(key: inclineFirst ? sideDeltOrderIdentityKey : (thirdSideDelt ? sideDeltIdentityKey : (shrugs ? shrugIdentityKey : (revised ? revisionIdentityKey : templateIdentityKey))), entries: [])]
        )
        try template.validate(
            exercisesById: Dictionary(uniqueKeysWithValues: exercises.map { ($0.id, $0) })
        )
    ...
    private static func squatProgressionKeys(template: CycleTemplate) -> [UUID: String] {
        guard [shrugVersionID, sideDeltVersionID, sideDeltOrderVersionID, chestBackVersionID].contains(versionID(for: template)) else { return [:] }
    ...
    static func resolvedSlots(
        ...
        CycleOrdering.sortedSlots(selection.day.slots).map { slot in
            ...
            return ResolvedSlot(
                slot: slot,
                exerciseId: exerciseId,
                progressionKey: balancedProgressionKey(selection: selection, slotPosition: slot.position, exerciseId: exerciseId)
    ...
    static func progressionKey(selection: Selection, slotPosition: Int) -> String {
        ...
        if let slot = selection.day.slots.first(where: { $0.position == slotPosition }),
           let key = balancedProgressionKey(selection: selection, slotPosition: slotPosition, exerciseId: slot.exerciseId) { return key }
        if selection.programVersionID == chestBackVersionID && selection.cluster == .cluster1 {
            return chestBackProgressionKey(step: step, slotPosition: slotPosition)
        }
        if [shrugVersionID, sideDeltVersionID, sideDeltOrderVersionID, chestBackVersionID].contains(selection.programVersionID) && selection.cluster == .cluster3 && slotPosition == 2 {
            return shrugProgressionKey
        }
        if selection.programVersionID == sideDeltOrderVersionID && selection.cluster == .cluster3 && slotPosition == 0 {
            if step % 3 == 0 { return sideDeltProgressionKey }
            return progressionKey(cluster: .cluster3, effectiveStep: step % 3 == 1 ? 0 : 1, slotPosition: 0)
        }
        if [sideDeltVersionID, chestBackVersionID].contains(selection.programVersionID) && selection.cluster == .cluster3 && slotPosition == 0 {
        ...
        if [revisionVersionID, shrugVersionID, sideDeltVersionID, sideDeltOrderVersionID, chestBackVersionID].contains(selection.programVersionID) {
